...
items-center
gap-2\>
            {step === 'email' ? <Mail className=\w-5
h-5
text-purple-400\ /> : <ShieldCheck className=\w-5
h-5
text-blue-400\ />}
            {step === 'email' ? 'Sign in to Origin' : 'Verify Identity'}
          </CardTitle>
          <CardDescription className=\text-zinc-400\>
            {step === 'email' 
...
text-xs
px-1\>
                   <span className=\text-zinc-500\>
                     {cooldown > 0 ? \Resend in \s\ : 'Ready for resend'}
                   </span>
                   <button 
                    type=\button\
                    onClick={() => setStep('email')}
                    className=\text-purple-400
